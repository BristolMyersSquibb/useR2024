...
new_ggplot_block <- function(col_x = character(), col_y = character(), ...) {
  ...
  new_block(
    fields = list(
      x = new_select_field(col_x, data_cols, title = "name"),
      y = new_select_field(col_y, data_cols, title = "name")
    ),
    expr = quote(
...
new_geompoint_block <- function(color = character(), shape = character(), ...) {
  ...
  new_block(
    fields = list(
      color = new_select_field(color, data_cols, title = "name"),
      shape = new_select_field(shape, data_cols, title = "name")
    ),
    expr = quote(
...
Create a new __lm()__ block: `lm(bill_length_mm ~ flipper_length_mm, data = penguins)`

## Create new blocks: lm block 1/4 {auto-animate=true}
...
## Create new blocks: lm block 2/4 {auto-animate=true}
...
new_lm_block <- function(y = character(), predictor = character(), ...) {
  ...
  sel_col <- function(col) \(data) data[[col]]
  all_cols <- function(data) colnames(data)

  fields <- list(
    y = new_select_field(sel_col(1), all_cols, title = "Y"),
    predictor = new_select_field(sel_col(2), all_cols, title = "Predictor")
  )
...
## Create new blocks: lm block 3/4 {auto-animate=true}
...
new_lm_block <- function(y = character(), predictor = character(), ...) {
  ...
  new_block(
    fields = fields,
    expr = quote({
      f <- as.formula(sprintf("%s ~ %s", .(y), .(predictor)))
      lm(f, data = data)
    }),
...
## Create new blocks: lm block 4/4 {auto-animate=true}
