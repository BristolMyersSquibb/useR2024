...
:::{.column}
We want `penguins %>% count(species)`
...
new_count_block <- function(column = character(), ...) {
  fields <- list(
    cols = new_string_field(column, type = "name") # <1>
  )
...
new_count_block <- function(column = character(), ...) {
  ...
  fields <- list(
    cols = new_string_field(column, type = "name")
  )
...
flowchart LR
  ...
  new_block1 --> |register| registry
  new_block2 --> |register| registry
```
:::
